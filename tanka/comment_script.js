...
document.getElementById('post-btn').addEventListener('click', async () => {
    ...
    formData.append('count', commentCount); // 💡 人数を送信

    try {
        const response = await fetch("/api/comment/tanka", { method: "POST", body: formData });
...
document.getElementById('save-image-btn').addEventListener('click', () => {
    ...
    html2canvas(target, {
        ...
        downloadLink.download = `AI短歌コメント欄_${new Date().toLocaleDateString()}.jpg`;
